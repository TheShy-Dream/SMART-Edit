...
class SMART_EditFireFlowEditFluxPipeline(RFEditingFluxPipeline):
    def denoise(
        self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
                ...
                if not with_second_order:
                    ...
                    latents = latents.to(dtype)  # latent用一阶演化，但是mid_noise保存用来估计latent，二阶预测了以后可以给之后的一阶使用
...
class SMART_EditFireFlowEditSD35Pipeline(RFEditingSD35Pipeline):
    def denoise(
        self,
        latents: torch.Tensor,
        pooled_prompt_embeds: torch.Tensor,
