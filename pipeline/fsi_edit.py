...
class RFFSIEditFluxPipeline(RFEditingFluxPipeline):
    def denoise(
        self,
      ...
      with self.progress_bar(total=num_inference_steps) as progress_bar:
        for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
            ...
            joint_attention_kwargs["noise_injecting_strategy"] = "inject_qk"
        ...
        if inverse: 
          for i in range(len(latent_list) - 1, 0, -1):
            latent_list[i] = freq_com(
              high=latent_list[i],   # 当前步 (较噪)
              low=latent_list[i-1],  # 上一步 (较干净)
              alpha=smooth_coeff,            # 这里的 alpha 含义取决于 freq_com 实现
              d_s=scale_coeff,
              d_t=scale_coeff
            )
          return latent_list
    ...
    def __call__(
      ...
      inverse_latents_list = self.denoise(
      ...
      latents = self.denoise(
          latents=inverse_latents_list[-1],
          pooled_prompt_embeds=target_pooled_prompt_embeds,
          prompt_embeds=target_prompt_embeds,
...
class RFFSIEditSD35Pipeline(RFEditingSD35Pipeline):
  def denoise(
      self,
      latents: torch.Tensor,
      pooled_prompt_embeds: torch.Tensor,
