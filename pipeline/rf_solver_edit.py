...
class RFSolverEditSD35Pipeline(RFEditingSD35Pipeline):
    def denoise(
        self,
        latents: torch.Tensor,
        pooled_prompt_embeds: torch.Tensor,
        ...
        num_warmup_steps = max(len(timesteps) - num_inference_steps * self.scheduler.order, 0) #processor_bae和inference_step差了1
        using_inject_list = [True] * inject_step + [False] * (len(timesteps[:-1]) - inject_step) #去最后的inject_step步保存他们的value值，这让刚开始的去噪效果更好
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
                sigma_curr = t_curr / self.scheduler.config.num_train_timesteps #这里统一了inversion和denoise的step_num
                sigma_prev = t_prev / self.scheduler.config.num_train_timesteps


                joint_attention_kwargs["inverse"] = inverse #是否invert
                joint_attention_kwargs["editing_strategy"] = "replace_v" #这些参数会被传到 AttnProcessor的__call__ FluxAttnProcessorWithMemory
                joint_attention_kwargs["second_order"] = False #二阶是否保存，原文好像是保存的
                joint_attention_kwargs["inject"] = using_inject_list[step_idx] #应该是在denoise的时候才会用，表示是否要inject,或者保存要inject的内容根据inverse参数来确定
                joint_attention_kwargs["timestep"] = int(t_prev.item()) if inverse else int(t_curr.item()) #输入timestep
                
                last_step_idx = len(timesteps) - 2
                if save_attn and ((inverse and step_idx == 0) or (not inverse and step_idx == last_step_idx)):
                    joint_attention_kwargs["save_attn"] = True
                else:
                    joint_attention_kwargs["save_attn"] = False
                ...
                if not with_second_order: #单步ODE
                    ...
                    mid_sample = latents + (sigma_prev - sigma_curr) / 2 * noise_pred #当前是t_{prev}-t_{curr} 对应curr的noise_pred 现在要变成t_{curr} + (t_{prev}-t_{curr})/2 使用这个时间步noise pred
                    #mid_sample = mid_sample.to(dtype)
                    ...
                    #latents = latents.to(dtype) # 二阶ODE估计
    ...
    def reconstruction(
        ...
        inverse_latents = self.denoise(
            ...
            device=device,
            joint_attention_kwargs=joint_attention_kwargs,
            negative_prompt_embeds=source_negative_prompt_embeds, # Pass negative
            negative_pooled_prompt_embeds=source_negative_pooled_prompt_embeds, # Pass negative
            inverse=True,
            with_second_order=with_second_order,
            callback_on_step_end=callback_on_step_end,
            callback_on_step_end_tensor_inputs=callback_on_step_end_tensor_inputs,
