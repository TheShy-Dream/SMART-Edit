...
class SMART_EditVanillaFluxPipeline(RFEditingFluxPipeline):
    def denoise(
        self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
                ...
                # Mask mechanism from smart_edit_flow_edit.py
...
class SMART_EditVanillaSD35Pipeline(RFEditingSD35Pipeline):
    def denoise(
        self,
        latents: torch.Tensor,
        pooled_prompt_embeds: torch.Tensor,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
                ...
                # Mask mechanism from val_flow_edit.py
                mask_container_hook = {}
                joint_attention_kwargs["attention_weight_control"] = True
                joint_attention_kwargs["mask_fusion_strategy"] = "fusion_v"
